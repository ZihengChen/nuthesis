...
Effectively, this parameterizes the efficiency matrix in Equation~\ref{eqn:analysis:method:effMatrix:eff_matrix} by the observables listed above, the number of jets, and the number of \PQb tags.  Having constructed the data model, the negative log likelihood can be calculated,
\begin{equation}
...
% included channels in the four categories are shown in Figure~\ref{efficencyMatrix}.
...
% $f$ with background subtracted. Based on Equation~\ref{eqn:analysis:method:effMatrix:data_model}, the measured normalized yields
...
% % Based on Equation~\ref{prediction}, the measured normalized yields $\{r_\Pe,r_\PGm,r_\PGt\}$ 
...
% explicit form of $\boldsymbol{E}$ and $\boldsymbol{B}$ matrices in Equation~\ref{eqn:analysis:method:effMatrix:br_matrix} and Equation~\ref{eqn:analysis:method:effMatrix:eff_matrix}
...
%     as is given in Equation~\ref{eqn:testinput}.
