...
%             =&  \Gamma_\ell \bigg[ 1+ \frac{\alpS(M_W)}{\pi} \bigg]  \sum_{color} \sum_{ i,j } |\mathrm{V}_{ij}|^2  \\
%             =&  \Gamma_\ell \bigg[ 1+ \frac{ \alpS(\mu_R) - \alpha^2_s(\mu_R) \frac{ \beta_0}{2\pi} \ln \frac{M_W}{\mu_R}}{\pi} \bigg] \sum_{color} \sum_{ i,j } |\mathrm{V}_{ij}|^2  
\end{split} .
\end{equation}
...
\begin{equation}
    \frac{\Gamma_{\rm had}^{\rm NLO}}{\Gamma_{\rm lep}} =  \underbrace{(1+\frac{\alpS}{\pi})}_{k} \sumCKM. %\sum_{ij=(uc)(dsb)} |\mathrm{V}_{ij}|^2.
\end{equation}
...
% and the renormalization scale is conventionally chosen as Z mass $\mu_R=M_Z$. The running couplings derived from the higher order of QCD renormalization group equation is discussed in the Section~\ref{sec:relatedWorks:qft:qcd}. The latest PDG value of $\alpS(m_Z)=0.1178\pm0.0010$. Using the leading order running of coupling, the \alpS at\PW pole is
% \begin{equation}
% 	\alpS(M_W) = 0.1199 \pm 0.0010
% \end{equation}
% By definition, the branching fraction of\PW satisfies the unitary constrain
% \begin{equation}
%     \sum_{ i,j } B_{\PW \to \PQq_i \PQq_j} + 3 B_\ell = 1
% \end{equation}

% \noindent using  $B_{\PW \to \PQq_i \PQq_j} = 3 k |\mathrm{V}_{ij}|^2 B_\ell $ to substitute $B_{\PW \to \PQq_i \PQq_j}$  with $B_\ell$, one gets
% \begin{equation}
%     \sum_{ i,j } |\mathrm{V}_{ij}|^2 = \frac{1}{ 3k} \, \frac{1-3B_\ell}{B_\ell}
% \end{equation}



% \noindent  With $\sum_{ i,j } |\mathrm{V}_{ij}|^2$ and the experimental value of other 5 better measured CKM elements \cite{pdg2020} in Table~\ref{tab:relatedWorks:vcs:ckm}, we can calculate the \absVcs.
